...
{
	ptFrameBuf->iPixFormat = (g_ptDefaultDispOpr->iBpp == 16) ? V4L2_PIX_FMT_RGB565 : \
                                   (g_ptDefaultDispOpr->iBpp == 32) ?  V4L2_PIX_FMT_RGB32 : \
                                           0;
    ptFrameBuf->iWidth  = g_ptDefaultDispOpr->iXres;
    ptFrameBuf->iHeight = g_ptDefaultDispOpr->iYres;
    ptFrameBuf->iBpp    = g_ptDefaultDispOpr->iBpp;
    ptFrameBuf->iLineBytes    = g_ptDefaultDispOpr->iLineWidth;
    ptFrameBuf->iTotalBytes   = ptFrameBuf->iLineBytes * ptFrameBuf->iHeight;
    ptFrameBuf->aucPixelDatas = g_ptDefaultDispOpr->pucDispMem;
	return 0;
}

void FlushPixelDatasToDev(PT_VideoBuf ptPixelDatas)
{

...
static int V4L2InitDevice(char * strDevName, PT_VideoDevice pt_VideoDevice)
{
    ...
    if (!(tV4l2Cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
		DBG_PRINTF("Error opening device: video capture not supported.\n");
		goto fatal;
    }
    ...
    tFmt.fmt.pix.pixelformat = pt_VideoDevice->iPixFormat ;
	...
	if (tV4l2Cap.capabilities & V4L2_CAP_STREAMING){
		...
		pt_VideoDevice->pucV4L2Buf[0]	= (unsigned char *)malloc(pt_VideoDevice->uiV4L2BufMaxLen);
	}
	...
	pt_VideoDevice->pt_VideoOpr = &g_tV4L2VideoOpr;
...
static int V4L2GetFrame(PT_VideoDevice pt_VideoDevice, PT_VideoBuf pt_VideoBuf)
{
	...
	if (pt_VideoDevice->ucStreamOrRW == STREAMING){
	    ...
	    pt_VideoBuf->iPixFormat        = pt_VideoDevice->iPixFormat;
	    pt_VideoBuf->iWidth  = pt_VideoDevice->iwidth;
	    pt_VideoBuf->iHeight = pt_VideoDevice->iHeigth;
	    pt_VideoBuf->iBpp    = (pt_VideoDevice->iPixFormat == V4L2_PIX_FMT_YUYV)  ? 16 : \
                               (pt_VideoDevice->iPixFormat == V4L2_PIX_FMT_MJPEG) ? 0 : \
                               (pt_VideoDevice->iPixFormat == V4L2_PIX_FMT_RGB565) ? 16: 0;
	    pt_VideoBuf->iLineBytes    = pt_VideoDevice->iwidth * pt_VideoBuf->iBpp / 8;
	    pt_VideoBuf->iTotalBytes   = tV4l2Buf.bytesused;
	    pt_VideoBuf->aucPixelDatas = pt_VideoDevice->pucV4L2Buf[tV4l2Buf.index];  
	}
	    ...
	    pt_VideoBuf->iBpp    = (pt_VideoDevice->iPixFormat == V4L2_PIX_FMT_YUYV) ? 16 : \
	                                        ...
	                                        (pt_VideoDevice->iPixFormat == V4L2_PIX_FMT_RGB565) ? 16 : 0;
	    pt_VideoBuf->iLineBytes    = pt_VideoDevice->iwidth * pt_VideoBuf->iBpp / 8;
	    pt_VideoBuf->iTotalBytes   = iRet;
	    pt_VideoBuf->aucPixelDatas = pt_VideoDevice->pucV4L2Buf[0];    
	}
...
static int V4L2StartDevice(PT_VideoDevice pt_VideoDevice)
{
    ...
    iError = ioctl(pt_VideoDevice->iFd, VIDIOC_STREAMON, &iType);
    if (iError < 0) {
...
static int V4L2GetFormat(PT_VideoDevice pt_VideoDevice)
{
	return pt_VideoDevice->iPixFormat;
}
